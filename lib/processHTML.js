module.exports = {
    ...
    getGrades: function(gradesTable){
        ...
        for(var i=2;i<gradesTable.length-1;i++){
            ...
            subject.extraExam = gradesTable[i].querySelectorAll('td')[12].innerHTML.trim();
            subject.retakeExam = gradesTable[i].querySelectorAll('td')[13].innerHTML.trim();
    ...
    getSemesterAverage: function(gradesTable){
        return gradesTable[gradesTable.length-1].querySelector('td:last-child').innerHTML.replace(/<\/?[^>]+(>|$)/g, ""); // To delete any inner html tag
    },
